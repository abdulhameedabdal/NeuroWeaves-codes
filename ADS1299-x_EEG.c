...
#if LOG_LOW_DETAIL == 1
  //NRF_LOG_INFO("Continuous Data Output Disabled %d..\r\n", rx_data_spi);
#endif
}
...
void ads1299_init_regs(ble_eeg_t *p_eeg, uint8_t *new_register_values) {
  ...
  memcpy(&p_eeg->ads1299_current_configuration[0], &new_register_values[0], num_registers);
  ...
  memcpy(&tx_data_spi[2], &new_register_values[0], num_registers);
...
void ads1299_init_regs_default(ble_eeg_t *p_eeg) {
	//	nrf_gpio_pin_clear(ADS1299_SPI_CS_PIN);nrf_gpio_pin_set(18);
	
  uint8_t err_code;
  uint8_t num_registers = 23;
  memcpy(&p_eeg->ads1299_current_configuration[0], &ads1299_default_registers[0], num_registers);
  ...
  tx_data_spi[0] = wreg_init_opcode;
  tx_data_spi[1] = num_registers - 1;
  memcpy(&tx_data_spi[2], &ads1299_default_registers[0], num_registers);
  spi_xfer_done = false;
  APP_ERROR_CHECK(nrf_drv_spi_transfer(&spi, tx_data_spi, txrx_size, NULL, NULL));
  while (!spi_xfer_done) {
    __WFE();
...
void ads1299_read_all_registers(ble_eeg_t *p_eeg) {
  ...
  for (uint8_t i = 0; i < 23; i++) {
    ...
    sum += b[i];
		//NRF_LOG_INFO("Register %d: Expected: 0x%x, Read: 0x%x \n", i, p_eeg->ads1299_current_configuration[i], rx_data_spi[2 + i]);
  }

  if (sum != 23) {
    NRF_LOG_INFO("ADS1299-4 - Data write failed! sum=%d \n", sum);
  ...
  memcpy(&p_eeg->ads1299_current_configuration[0], &rx_data_spi[2], 23);
}
...
void get_eeg_voltage_array_4ch(ble_eeg_t *p_eeg) {
  ...
  memcpy(&p_eeg->eeg_ch1_buffer[p_eeg->eeg_ch1_count], &rx_data[3], 3);
  memcpy(&p_eeg->eeg_ch2_buffer[p_eeg->eeg_ch1_count], &rx_data[6], 3);
  memcpy(&p_eeg->eeg_ch3_buffer[p_eeg->eeg_ch1_count], &rx_data[9], 3);
  memcpy(&p_eeg->eeg_ch4_buffer[p_eeg->eeg_ch1_count], &rx_data[12], 3);
